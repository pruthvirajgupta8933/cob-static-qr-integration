...
const MyMerchantList = () => {
  ...
  const myMerchantListDataa = [
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="removeWhiteSpace">{row?.login_id?.name}</div>
      ),
      width: "200px",
    },

    {
      ...
      cell: (row) => (
        <div className="removeWhiteSpace">{row?.login_id?.email}</div>
      ),
      width: "200px",
    },
    {
      ...
      <div className="form-row">
        ...
        <div className="form-group col-lg-3 col-md-12 mt-2">
          ...
          <select
            ...
            onChange={(e) => KycStatusHandler(e.currentTarget.value)}
          >
            {kycStatus?.map((d, i) => (
              <option
                value={d?.key}
                key={uuidv4()}
                selected={kycSearchStatus === d.key}
              >
                {d.values}
              </option>
            ))}
      ...
      <div>
        <div className="scroll overflow-auto">
          ...
          <h6>
            Total Count (
            {kycSearchStatus
              ? kycStatus.find((item) => item.key === kycSearchStatus)?.values
              : ""}
            ) : {dataCount}
          </h6>
