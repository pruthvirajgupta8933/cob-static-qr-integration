...
const OnboardMerchant = ({ zoneCode, heading, clientLoginId, validator }) => {
  ...
  const handleSubMerchant = (formData, { resetForm }) => {
    let { fullname, mobilenumber, emaill, passwordd, username } = formData;
    setBtnDisable(true);
    dispatch(
      createSubMerchant({
        mobile: mobilenumber,
      ...
      .then((res) => {
        if (res?.status === true) {
          ...
          history.push(
            `kyc?kycid=${stringEnc(res.sub_merchant?.loginMasterId)}`
          );
        } else toastConfig.errorToast(res?.detail ?? res?.message);
        setBtnDisable(false);
      })
      .catch((err) => {
        toastConfig.errorToast(err.message);
        setBtnDisable(false);
      });
  };
