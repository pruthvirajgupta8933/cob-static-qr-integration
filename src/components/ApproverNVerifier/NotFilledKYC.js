...
const NotFilledKYC = () => {
  ...
  const notFilledMerchants = async () => {
    await axiosInstanceAuth.get(`${API_URL.KYC_FOR_NOT_FILLED}`)
      .then(res => {
        const data = res?.data?.results;
        // console.log("Not Filled Data", data)
  ...
  useEffect(() => {
    notFilledMerchants();
    dispatch(kycForNotFilled({ page: currentPage, page_size: pageSize })).then((resp) => {
