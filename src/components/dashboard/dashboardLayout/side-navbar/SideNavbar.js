...
function SideNavbar() {
  ...
  // QR feature flag - only enable in development or for specific test users
  const isQRFeatureEnabled = process.env.NODE_ENV === 'development' || 
                             process.env.REACT_APP_ENABLE_QR_FEATURE === 'true' ||
                             ["795", "31407"].includes(auth?.user?.loginId?.toString()); // Only for test users
    ...
    >
      {!menuListReducer?.isLoading ? (
        <div className="position-sticky pt-3 pl-3">
          ...
          {/* QR Solutions Menu - Positioned before Latest updates */}
          {true && (
            <>
              <div onClick={(e) => toggleMenu(e)}>
                <h6
              ...
              <ul
                className={`nav flex-column mb-2 ml-2 ${sideNavClasses.submenu_border}`}
                role="menu"
              >
                <li className="nav-item ml-1" role="menuitem">
                  <Link
                    to={`${url}/static-qr`}
                    className={`nav-link ${sideNavClasses.sidebar_menu} ${
                      selectedMenu === "static-qr" ? sideNavClasses.selected_memu : ""
                    }`}
                  >
