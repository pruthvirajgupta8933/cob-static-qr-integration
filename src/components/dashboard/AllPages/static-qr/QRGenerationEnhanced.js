...
import QRTemplateDesign from './components/QRTemplateDesign';
...
// Component for live preview that uses Formik context
const FormikLivePreview = ({ customIdentifier, amountType, merchantName }) => {
    const { values } = useFormikContext();
    const [canvasRef, setCanvasRef] = useState(null);
    
    return (
        <QRTemplateDesign 
            identifier={customIdentifier || values.qr_identifier}
            merchantName={values.reference_name || merchantName}
            reference={values.description || ''}
            amount={amountType === 'fixed' ? values.max_amount_per_transaction : null}
            qrData={null}
...
const QRGenerationEnhanced = () => {
    ...
    const merchantName = user?.clientMerchantDetailsList?.[0]?.merchantName || 'SRS Live Technologies';
    const merchantPrefix = VPAGenerator.generateMerchantPrefix(merchantName);
    ...
    const handleGenerateQR = async (values, { setSubmitting, resetForm }) => {
        try {
            ...
            const qrPayload = {
                ...
                merchant_name: merchantName, // Pass the merchant name for VPA generation
                                    ...
                                    <div className="form-group">
                                        ...
                                        <div className="input-group input-group-lg">
                                            <div className="input-group-prepend">
                                                <span className="input-group-text bg-primary text-white">sabpaisa.{merchantPrefix}</span>
                                            </div>
                                            <input
                                                type="text"
                                            ...
                                            <div className="input-group-append">
                                                <span className="input-group-text bg-primary text-white">@hdfcbank</span>
                                            </div>
                                        </div>
                                        {customIdentifier && customIdentifier.length === 5 && identifierValidation?.available === true && (
                                            <small className="text-success d-block mt-2">
                                                <i className="fa fa-check-circle mr-1"></i>
                                                Your VPA will be: <strong>sabpaisa.{merchantPrefix}{customIdentifier.toLowerCase()}@hdfcbank</strong>
                                            </small>
                                        )}
