    ...
    case "financialYear":
      return {
        fromDate: moment()
          ...
          .date(1)
          .subtract(1, "year")
          .format("YYYY-MM-DD"), // April 1st of the previous year
...
const FilterModal = ({ show, onClose, filterRef, onApply }) => {
  ...
  const initialValues = {
    fromDate: dateFilterData?.fromDate || today,
    toDate: dateFilterData?.toDate || today,
    dateRange: dateFilterData?.dateRange || "today",
  };
  ...
  return (
    <div
      ref={modalRef}
      className={` ${Classes.filter_modal_open} position-absolute bg-white border shadow p-3 rounded`}
    >
      <h6>Select Range</h6>
      <Formik
