...
const Profile = () => {
  ...
  const { auth, productCatalogueSlice, subscription } = useSelector(
    (state) => state
  );
  const { user } = auth;
  const { SubscribedPlanData, isLoading, walletCommission, errorState } =
  ...
  useEffect(() => {
    ...
    merchantSubscriptionList?.result?.map((data, i) =>
      dataWallet.push(
        <div className="col-lg-4 mx-3 my-1" key={uuidv4()}>
          <div className="card" style={{ width: "18rem" }}>
            <div className="card-body">
              ...
              {isNull(data?.mandateStatus) &&
                data?.mandateStatus?.toLowerCase() !== "success" &&
                data?.plan_code === "005" && (
                  <p className="text-danger"> Payment is pending </p>
                )}
                ...
                data?.plan_code === "005" && (
                  <>
                    ...
                    <p className="card-text">
                      Subscribed Date :{" "}
                      {moment(data.mandateStartTime).format("DD/MM/YYYY")}{" "}
                    </p>
                    <p className="card-text">
                      Purchased Amount : {data.purchaseAmount} INR
                    </p>
                    <p className="card-text">
                      Commission : {data.commission ?? 0} INR
                    </p>
                    <p className="card-text">
                      Wallet Balance :{" "}
                      {balanceCalculate(data.purchaseAmount, data.commission)}{" "}
                      INR
                    </p>
                  </>
                )}
