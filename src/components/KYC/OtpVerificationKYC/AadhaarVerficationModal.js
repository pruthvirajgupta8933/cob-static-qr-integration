...
const AadhaarVerficationModal = ({
  ...
  const { aadharOtpResponse } = useSelector(
    (state) => state.kycValidatorReducer
  );
  ...
  const aadharOtpVerification = async () => {
    ...
    try {
      const resp = await kycValidatorAuth.post(API_URL.Aadhar_otp_verify, {
        referenceId: aadharOtpResponse?.referenceId,
    ...
    return (
      <React.Fragment>
        ...
        <ErrorMessage name="aadhaarOtpDigit">
          {(msg) => <p className="text-danger text-center m-1">{msg}</p>}
        </ErrorMessage>
  ...
  return isOpen ? (
    <CustomModal
      ...
      modalBody={aadharModalBody}
      modalToggle={isOpen}
      fnSetModalToggle={() => modalCloseHandler()}
      modalSize="modal-md"
    />
  ) : (
    <></>
