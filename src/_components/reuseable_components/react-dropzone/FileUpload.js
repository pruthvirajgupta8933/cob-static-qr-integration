      ...
      {!isEditableInput && (
        <div className="file-uploader">
          ...
          <div {...getRootProps()} className="dropzone">
            <input {...getInputProps()} />
            <p>
              Drag 'n' drop some PDF files here, or click to select files 5MB
            </p>
          </div>
          {uploadedFiles?.length > 0 && (
            <button
              onClick={uploadFiles}
              className="upload-button btn cob-btn-primary btn-sm mt-2"
            >
              {submitLoader && (
                <>
                  <span
          ...
          {documentUploadResponse === true && (
            <a
              className="btn active-secondary btn-sm m-2 mt-3"
              href={() => false}
