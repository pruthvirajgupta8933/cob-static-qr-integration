  ...
  axiosInstanceJWT.interceptors.response.use(
    ...
    async (error) => {
      ...
      if (error.response.status === 401 && !originalRequest._retry) {
        ...
        try {
          const res = await fetch(API_URL.BASE_URL_COB + "/auth-service/auth/refresh-token", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ refresh: TokenService.getLocalrefreshToken() }),
          });

          if (res.ok) {
            const data = await res.json();
            TokenService.updateLocalAccessToken(data.access);
            axiosInstanceJWT.defaults.headers.common["Authorization"] = 'Bearer ' + data.access;
            return axiosInstanceJWT(originalRequest); // Retry the original request with the new token
          }
      ...
      return Promise.reject(error); // If the error is not a 401 or if it's a retry, reject the promise
    }
  );
