...
const sessionDataI = JSON.parse(
  localStorage.getItem("onboardingStatusByAdmin")
);
const initialState = {
  isLoading: false,
...
export const fetchParentTypeData = createAsyncThunk(
  ...
  async (requestParam, thunkAPI) => {
    const response = await fetchPerentTypeMerchantData(requestParam).catch(
      (error) => {
...
export const updateBasicDetailsSlice = createAsyncThunk(
  ...
  async (requestParam, thunkAPI) => {
    const response = await updateBasicDetails(requestParam).catch((error) => {
      return error.response;
    });
...
export const saveBankDetails = createAsyncThunk(
  ...
  async (requestParam) => {
    const response = await bankDetails(requestParam).catch((error) => {
      return error.response;
    });
    return response.data;
  }
...
export const getAllZoneName = createAsyncThunk(
  ...
  async (requestParam) => {
    const response = await getAllZoneCode(requestParam).catch((error) => {
      return error.response;
    });
    return response.data;
  }
...
export const businessDetailsSlice = createAsyncThunk(
  ...
  async (requestParam) => {
    const response = await saveBusinessDetails(requestParam).catch((error) => {
      return error.response;
    });
    return response.data;
  }
...
export const merchantReferralOnboardSlice = createSlice({
  ...
  reducers: {
    ...
    updateOnboardingStatus: (state, action) => {
      ...
      state.merchantOnboardingProcess.isOnboardStart =
        action.payload?.isOnboardStart;
      state.merchantOnboardingProcess.merchantLoginId =
        action.payload?.merchantLoginId;
      ...
      localStorage.setItem(
        "onboardingStatusByAdmin",
        JSON.stringify(onboardingStatusComplete)
      );
    },
  },
    ...
    builder
      ...
      .addCase(saveMerchantBasicDetails.fulfilled, (state, action) => {
        ...
        if (action.payload.merchant_data?.status === "Activate") {
          state.merchantOnboardingProcess.merchantLoginId =
            action.payload.merchant_data?.loginMasterId;
          ...
          localStorage.setItem(
            "onboardingStatusByAdmin",
            JSON.stringify(onboardingStatusByAdmin)
          );
        }
      })
...
export const {
  clearErrorMerchantReferralOnboardSlice,
  resetStateMfo,
  updateOnboardingStatus,
} = merchantReferralOnboardSlice.actions;
